		...
		for (divider = start; divider < end && isSimple; divider++)
		{
			...
			#ifdef DEBUG
			{
				if (rank == 1)
				{
					...
					if (timeProc - timeProcStart > 1.f)
					{
						uint64_t step = divider - start;
						int per = (long double)step / length * 100;
						sprintf_s(msg, "Process running... [%llu/%llu] (%d%%): %f", step, length, per, timeProc - timeStart);
						MPI_Send(&msg, BUFSIZE, MPI_CHAR, MASTER, TAG_MSG, MPI_COMM_WORLD);
